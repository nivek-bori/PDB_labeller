...
def create_pointpillars_config(data_dir_relative_path: str, point_cloud_range: list):
    return f"""
CLASS_NAMES: ['Car', 'Pedestrian', 'Cyclist']

DATA_CONFIG: 
    _BASE_CONFIG_: {os.path.join(data_dir_relative_path, "custom_dataset.yaml")}
    POINT_CLOUD_RANGE: {point_cloud_range}
    DATA_PROCESSOR:
        - NAME: mask_points_and_boxes_outside_range
          REMOVE_OUTSIDE_BOXES: True
...
def write_config(path: str, dataset_config: str, model_config: str):
    safe_makedirs(path, exist_ok=True)
    with open(os.path.join(path, "custom_dataset.yaml"), "w") as f:
        f.write(dataset_config)
    with open(os.path.join(path, "model.yaml"), "w") as f:
